...
This repository contains the code and data for the final project. The analysis is implemented as a Python script that reads the required input datasets from the `data/` directory, executes the full workflow, and saves select generated outputs (including plots, variable lists and function returns) to the `output/` directory. A `requirements.txt` file is provided to ensure reproducibility.
